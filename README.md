...
- **Annotation pipeline**, run once per document (and cached per language after that):
  ...
  3. Gemini reads the de-identified text and picks out the key findings a layperson would need explained. For each it proposes structured pieces — the standard literature name for the concept (not a plain-language paraphrase), synonyms, and whether the term is generic enough that the exam type is needed to pin down its meaning — plus the document's own scan type and body region. It never writes the search itself.
  4. Real papers are retrieved from PubMed (NCBI E-utilities), and this is the *only* source material the model is ever allowed to cite. `documents/pubmed.py` builds the boolean query in code from those pieces, with guaranteed filters (human subjects, has an abstract, English) so a bare word like "uptake" doesn't return mouse-model papers; the exam context is only applied to terms flagged as generic, since adding it to a specific term ("hyponatremia") only drags in unrelated papers. A reviews/guidelines-only variant of the same query is unioned in so explanatory papers are in the pool, then candidates are reranked deterministically (study type, human subjects, recency, concept overlap). A relevance grader (one batched Gemini call) then keeps only papers whose central subject is the concept itself, in humans.
  5. A second Gemini call writes the plain-language summary and per-finding explanations, and for each citation must return an `evidence_quote` copied verbatim from that paper's abstract. Before anything reaches a reader, `services._verify_citations` checks the PMID is a real retrieved candidate *and* that the quote actually appears in that abstract; anything else is dropped, so a term with no genuinely supporting paper is shown as uncited rather than with a plausible-looking one. The explanations are written once, always in English — the one and only language Gemini ever generates. Every other supported language is *translated* from that canonical English version via Cloud Translation (`documents/translate.py`), not independently regenerated — cheaper, near-instant, and (the actual point) guaranteed to describe each finding the same way in every language, rather than risking a second LLM pass that subtly diverges in what it chooses to emphasize. `Document.findings`' own `term` field, and every citation's title, pass through translation untouched: the term has to stay the literal string from the source document or highlighting breaks, and a citation title is already the real paper's own title regardless of annotation language. `DocumentViewSet.perform_create` generates + translates every supported language eagerly at upload time, so switching languages in the viewer is a cache hit, not a new API call — see `documents/services.py`'s `get_or_create_annotation`.
...
- **On-demand explanations**, for anything the automatic pass didn't pick: select any text in the document (or type a phrase into the box below the findings list) and `documents/services.py`'s `explain_ad_hoc_term` runs the same retrieval-then-generation pipeline — PubMed search, a grounded Gemini call (English only, then translated — same rule as the automatic pass above), citation re-validation — for just that one term. The result is immediately propagated to *every* language this document already has cached annotations for, not just whichever one the reader happened to be viewing when they asked — so asking about a term while reading in Spanish means it's already there, identically explained, if they switch to English a minute later. Persisted the same way any other finding is (`Document.findings` / `Annotation.items`) so it's highlighted and listed from then on with no special-casing on the frontend. Unlike the automatic pass, which only ever lets Gemini see already-redacted text, a selected phrase can be anything in the original document — so it's run through the same four-layer redaction pipeline *itself* before ever reaching PubMed or Gemini; if that pipeline would redact anything in it, the request is refused rather than sent.
...
Live in production at [app.plainmed.health](https://app.plainmed.health) (invite-only): auth, upload/storage, the PDF viewer, the automatic + on-demand annotation pipeline, multi-language support, and the dashboard UI are all built and deployed. Security hardening is in place too — Cloud DLP, CMEK-encrypted storage, a private-IP-only database, Vertex AI, a per-document access audit trail (distinct from GCP's own infra-level audit logging — see [How it works](#how-it-works)), and a Cloudflare WAF in front of the custom domain (managed ruleset, Bot Fight Mode, and rate limiting on the sign-in and on-demand-explain endpoints — see [Cloudflare WAF setup](#cloudflare-waf-setup)) — see [Deploying to GCP](#deploying-to-gcp). Remaining: signing the actual BAA with Google (a manual step outside this codebase — see [Before real patient data touches this](#before-real-patient-data-touches-this)).
...
- `PUBMED_API_KEY` — optional, raises PubMed from 3 to 10 requests/sec (free key from your NCBI account). Worth setting in production too (`pubmed_api_key` in `terraform.tfvars`): the wider candidate retrieval makes more requests per upload, and a realistic ~10-finding report took ~26s end to end without a key, against a 60s Gunicorn worker timeout.
...
- `TRANSLATE_PROJECT_ID` — needed to test anything beyond English locally (Spanish/Traditional Chinese annotations are translated, not regenerated — see [How it works](#how-it-works)). Requires `gcloud auth application-default login` with `roles/cloudtranslate.user` on that project.
...
Puts a free Cloudflare proxy in front of the custom domain from the previous section — a managed WAF ruleset, Bot Fight Mode, and edge rate limiting, all before a request reaches Cloud Run. Requires [Custom domain setup](#custom-domain-setup) already done: this proxies the *existing* domain mapping rather than replacing it.

**Why proxy the domain mapping instead of bypassing it**: the first version of this setup tried pointing Cloudflare straight at each Cloud Run service's own `*.run.app` host and using a Cloudflare Origin Rule to override the origin-leg SNI, to avoid Cloud Run's domain-mapping cert renewal (below). That doesn't work on Cloudflare's Free plan — Origin Rules' "Override SNI" action is Enterprise-only. Proxying the existing domain mapping instead needs no SNI override at all, since it already has a valid, Google-managed cert for the custom domain itself.

1. **Create a free Cloudflare account and add your domain as a site** ([dash.cloudflare.com](https://dash.cloudflare.com)) — Free plan. Cloudflare scans existing DNS and offers to import it; you can let that finish, but the records that matter are the ones `cloudflare.tf` creates below, not the scanned import.
2. **Get a scoped API token**: My Profile → API Tokens → Create Custom Token, permissions `Zone:DNS:Edit`, `Zone:Zone Settings:Edit`, `Zone:Firewall Services:Edit`, `Zone:Zone WAF:Edit`, `Zone:Bot Management:Edit`, scoped to this one zone — not the account-wide Global API Key. (The exact permission names Cloudflare's dashboard uses can drift; search the permission picker for "WAF" and "Bot" if these don't match exactly.)
3. **Get the Zone ID** from the site's Overview page (right sidebar, API section).
4. Set `cloudflare_api_token` and `cloudflare_zone_id` in `terraform.tfvars`, then `terraform apply`. This creates (`cloudflare.tf`):
   - `app`/`api` CNAME records pointed at `ghs.googlehosted.com` (the same target the Google-side domain mapping uses), proxied.
   - Mailgun MX/SPF/DKIM and DMARC records, matching `dns.tf`'s Google-side originals — unproxied (Cloudflare only proxies A/AAAA/CNAME anyway).
   - Zone TLS settings: `ssl = "strict"`, `min_tls_version = "1.2"`, and **`always_use_https = "off"`** — deliberately off, not an oversight. [Google's own docs for Cloud Run domain mappings](https://cloud.google.com/run/docs/mapping-custom-domains) call out Cloudflare by name: leaving "Always Use HTTPS" on can redirect Google's periodic cert-renewal validation request instead of passing it through, silently breaking renewal.
   - Bot Fight Mode (`fight_mode` + `enable_js`, both required together).
   - The WAF managed ruleset — **"Cloudflare Managed Free Ruleset"** specifically, not the commonly-referenced "Cloudflare Managed Ruleset"/"OWASP Core Ruleset" (those are Pro-plan-and-up; confirmed by querying this zone's actual entitled rulesets via `GET /zones/{zone_id}/rulesets` after the Pro-tier IDs were rejected outright). Free-plan coverage is narrower than a paid WAF — it's a curated set of known exploit signatures, not broad SQLi/XSS heuristics, confirmed by testing basic injection payloads against a live deployment and watching them pass through unblocked.
   - A single rate limit rule (Free/Pro allow exactly one in this phase) covering both `POST /api/auth/request-link/` and any `.../explain/` path — `period`/`mitigation_timeout` are locked to `10` (seconds) on this plan tier, confirmed by the API rejecting `60`/`300` outright.
5. **If any of the `app`/`api`/Mailgun records already exist** (from step 1's DNS scan), Terraform will fail with `expected DNS record to not already be present but already exists` — `terraform import "cloudflare_record.<name>[0]" "<zone_id>/<cloudflare_record_id>"` (record IDs from `GET /zones/{zone_id}/dns_records`) rather than deleting and recreating them.
6. **Update your registrar's nameservers**: `terraform output cloudflare_name_servers`, and **replace** (not add to) the existing Google Cloud DNS nameservers (`terraform output dns_name_servers`) with these two. A domain can only have one authoritative DNS provider — mixing both leaves resolution inconsistent depending on which nameserver a given resolver happens to query. Cloudflare's zone status flips from "Pending" to "Active" once it recognizes the change (minutes to a few hours).
7. **Verify**: `dig app.plainmed.health` resolves to a Cloudflare IP (not Google's directly), the padlock is valid in a browser, sign-in email still arrives, and a rapid burst of requests to `/api/auth/request-link/` gets a `429` after the second one.
...
- **WAF coverage gap**: the Free-plan managed ruleset above doesn't do broad SQLi/XSS-style heuristic blocking — that tier of coverage is Cloudflare Pro and up. If that gap ever matters more than it does at family scale, upgrading the zone's plan (no infrastructure change needed) or reconsidering the load balancer + Cloud Armor setup this deployment traded away for cost are both still options.
